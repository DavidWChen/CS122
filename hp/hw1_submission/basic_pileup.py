...
def generate_consensus(aligned_fn):
    ...
    with open(aligned_fn, 'r') as input_file:
        ...
        # snps.append(['','','0'])
        # #snps = ['','','0'] + snps
        # for x in range(0, len(snps)-1):
        #     if (int(snps[x][2])+1) != (int(snps[x+1][2])): #and (int(snps[x][2])+1) != (int(snps[x-1][2])-1): #This checks after, but not behind
        #         cleaned_snps+=[snps[x]]
        #         cleaned_output_lines+=[output_lines[x]]
        #         cleaned_output_lines+=[output_lines[x+1]]
        # # print(cleaned_snps)
        # # print(output_lines)
        # snps = cleaned_snps
...
def consensus(ref, reads):
    ...
    for i in range(len(ref)):
        ...
        ###
        # if len(read_bases)!=0:
        #     base_candidate = max(base_count.keys(), key=(lambda key: base_count[key]))
        #     #print(base_candidate)
        #     print(base_count[base_candidate])
        #     print(len(read_bases))
        #     if base_count[base_candidate]/len(read_bases)>0.9:
        #         consensus_base = base_candidate
        #     else:  
        #         consensus_base = ref_base
        ###
        consensus_base = max(base_count.keys(), key=(lambda key: base_count[key]))
        ###
        # if ref_base != consensus_base:
        #     # print (ref_base)
        #     # print (consensus_base)
        #     if (consensus_base == 'A' or consensus_base == 'T') and (ref_base == 'A' and ref_base == 'T'):
        #         # print('ok')
        #         pass
        #     elif (consensus_base == 'C' or consensus_base == 'G') and (ref_base == 'G' or ref_base == 'C'):
        #         # print('ok')
        #         pass
        #     else:
        #         # print("diff!")
...
if __name__ == "__main__":
    ...
    with open(output_fn, 'w') as output_file:
        ...
        snps.append(['','','0'])
        snps = [['','','0']] + snps
        for x in range(1, len(snps)-1):
            if (int(snps[x][2])+1) != (int(snps[x+1][2])) and (int(snps[x][2])-1) != (int(snps[x-1][2])): #This checks after, but not behind
                cleaned_snps+=[snps[x]]
        snps = cleaned_snps
